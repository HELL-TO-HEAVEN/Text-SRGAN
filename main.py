...
def train():
    ...
    ###vgg_loss = 5e-2 * tl.cost.mean_squared_error(vgg_predict_emb.outputs, vgg_target_emb.outputs, is_mean=True)
    EPSILON = 1e-6
    rho_loss = 1e-1 * tf.reduce_mean(tf.sqrt(tf.square(tf.subtract(net_g.outputs, t_target_image)) + EPSILON))
    g_loss = mse_loss + rho_loss + g_gan_loss
...
#            tl.vis.save_images(out, [ni, ni], save_dir_ginit + '/train_%d.png' % epoch)
    ...
    for epoch in range(0, n_epoch + 1):
        ...
        for idx in range(0, len(train_hr_imgs), batch_size):
            ...
            ## update G
            errG, errM, errV, errA, _ = sess.run([g_loss, mse_loss, rho_loss, g_gan_loss, g_optim], {t_image: b_imgs_96, t_target_image: b_imgs_384})
...
def evaluate(mode):
    ...
    for imid in range(num_lr_imgs): #64  # 0: 企鹅  81: 蝴蝶 53: 鸟  64: 古堡
        ...
        res_imp, bic_imp = ocr.getAccuracy(out_uint8, valid_hr_img, valid_lr_img, out_bicu, imid)
        tot_psnr += img_psnr
        tot_mse += img_mse
        tot_ssim += img_ssim
        tot_rel_vs_bic += res_imp/bic_imp
        ...
        #sys.stdout.flush()
    hist = "Average PSNR: "+str(tot_psnr/num_lr_imgs)[:8]+"\tAverage MSE: "+str(tot_mse/num_lr_imgs)[:8]+"\tAverage SSIM: "+str(tot_ssim/num_lr_imgs)[:8]+"\tAverage OCR accuracy compared to bicubic: "+str(tot_rel_vs_bic/num_lr_imgs)[:8]+'\n'
    history.write(hist)
    latest.close()
    history.close()
    print("\nAll images done\n"+hist)
